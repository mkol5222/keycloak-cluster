...
class WebsiteUser(HttpUser):
    ...
    def get_users(self):
        ...
        r = self.client.post("http://localhost:8000/auth/realms/master/protocol/openid-connect/token", data= {"client_id": "admin-cli", "username": "admin", "password": "admin", "grant_type": "password"}, headers={"Connection": "close"}).text
        # print(r)
        h =  {"Authorization": "Bearer "+json.loads(r)["access_token"], "Connection": "close"}
        self.client.get("http://localhost:8000/auth/admin/realms/master/users", headers=h, verify=False)
